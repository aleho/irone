 ...
 * android.text=[null] | []
 * android.title=[null] | []
 * android.textLines=[Ljava.lang.CharSequence;@6f5ee2 | [[Ljava.lang.CharSequence;]
 * android.subText=email@domain.net • Notifications | [java.lang.String]
 * android.summaryText=email@domain.net • Notifications | [java.lang.String]
 */
internal class InboxExtractor : AppExtractor()
{
    override fun doExtract(an: AppNotification, notification: Notification)
    {
        val summary = getSummary(notification)

        if (summary != null) {
            val parts = summary.split(" • ", limit = 2)
            if (parts.size == 2) {
                an.primary = an.application
                an.secondary = parts[1]
                an.application = null
            }

        } else {
            // "normal" notification, not grouped
